<template>
  <div id="dialog-demo">
    ...
    <button class="dialog-demo-button-1" @click="clickView" :focusable="true" :requestFocus="true">
      <span class="button-text" duplicateParentState>显示对话框</span>
    </button>
    <!-- dialog 无法支持 v-show，只能使用 v-if 进行显示切换 -->
    <dialog animationType="slide" class="dialog-demo" :supportedOrientations="supportedOrientations"
      v-if="dialogIsVisible" @show="onShow" @requestClose="onClose">
      <!-- dialog 里的布局比较奇怪，碰到问题请联系终端同学 -->
      <div class="dialog-demo-wrapper">
        <!-- 空白区域点击关闭 -->
        <div class="d-fullscreen" @click="clickView">
          <!-- 内容区域阻止点击事件冒泡导致关闭 -->
          <div class="dialog-demo-close-btn-2" @click="stopPropagation">
            <p class="dialog-demo-close-btn-text">按返回键关闭</p>
            <button class="dialog-demo-button-1" @click="clickOpenSecond" :focusable="true">
              <span class="button-text" duplicateParentState>点击打开二级全屏弹窗</span>
            </button>
          </div>
