<template>
  <div id="demo-swiper">
    ...
    -->
    <swiper id="swiper" ref="swiper" needAnimation :current="currentSlide" @dragging="onDragging" @dropped="onDropped"
      @stateChanged="onStateChanged">
      <!-- slides -->
      <swiper-slide v-for="n in dataSource" :key="n" :style="{ backgroundColor: 4278222848 + 100 * n }"
        class="swiper-slide">
        <p>I'm Slide {{ n + 1 }}</p>
      </swiper-slide>
    </swiper>
    <!-- 一个放小点的范例 -->
    <div id="swiper-dots">
      <div class="dot" v-for="n in dataSource" :key="n" :class="{ hightlight: currentSlideNum === n }" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    listener(key) {
      console.log('swiper:keydown', key)
      if (key.keyCode === 21) {
